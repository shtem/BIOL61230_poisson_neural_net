...
def plot_nn_architecture(model, model_name: str, base_dir="data/results"):
    """Visualise a PyTorch neural network and save the graph to disk.

    This routine uses :mod:`torchviz` to produce a directed graph of the
    network's computation graph, including layer names and parameter counts.
    The resulting PNG file is written to ``{base_dir}/{model_name}/architecture.png``
    so that it sits alongside the other diagnostic plots generated by
    :func:`run_experiment`.

    Parameters
    ----------
    model : torch.nn.Module
        The network whose structure should be drawn.  The function will attempt
        to infer the input dimensionality from ``model.input_dim`` or by
        inspecting the first ``nn.Linear`` layer.
    model_name : str
        Used to create a subdirectory under ``base_dir`` and to name the file.
    base_dir : str or Path, optional
        Root directory under which the architecture image is saved (default
        ``"data/results"``).
    ...
    ------
    ImportError
        If ``torch`` or ``torchviz`` cannot be imported.
    ValueError
        If the input dimension cannot be inferred from the model.
    """
    try:
        import torch
        ...
        raise ImportError(
            "torch and torchviz are required for plotting network architectures"
        ) from exc

    # infer input dimension
    if hasattr(model, "input_dim"):
        inp = torch.zeros(1, model.input_dim)
    else:
        # fall back to first linear layer we can find
        inp_dim = None
